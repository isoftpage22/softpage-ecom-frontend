...
const ContactFields = (props) => {
  const { formik, handleDigitsChange, busy } = props;
  return (
    <>
      <Stack spacing={8}>
            ...
            <FormControl>
              <InputGroup>
                <InputLeftElement
                  w={8}
                  h={8}
                  children={<Icon color={"whatsapp.500"} h={20} mt={2} w={6} as={RiWhatsappFill} />}
                />
                <Input
                  h={45}
                  ...
                  onChange={(e) => {
                    e.target.name = "whatsAppNumber";
                    handleDigitsChange(e, 10);
                  }}
