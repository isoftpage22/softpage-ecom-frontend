...
export function filterCatalogBySearch(productList, query) {
  const q = String(query || "").trim().toLowerCase()
  if (!q || !productList) return productList
  const categories = (productList.categories || [])
    .map((category) => {
      const categoryHit = String(category.categoryName || "").toLowerCase().includes(q)
      const products = (category.products || []).filter((product) => {
        if (categoryHit) return true
        const name = String(product.productName || product.name || "").toLowerCase()
        const desc = String(product.productDesc || product.description || "").toLowerCase()
